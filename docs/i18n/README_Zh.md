...
**DayPass** 是一个轻量、自动化、模块化的框架，专为 **OpenWrt** 路由器设计，用于简化网络管理、多条互联网连接的组合（**Multi-WAN**）以及不同路径之间的负载均衡。

当官方 OpenWrt 软件源无法访问或软件包下载出现问题时，DayPass 可以每天自动从 **SourceForge** 获取所需的软件包并准备好供下载。

DayPass 同时支持新版 OpenWrt 使用的两种软件包管理器：

- **OpenWrt 24.x** 使用 `opkg`
- **OpenWrt 25.x** 使用 `apk`

软件包会根据 OpenWrt 版本和路由器的硬件架构进行下载和安装。
