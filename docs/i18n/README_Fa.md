...
ابزار **DayPass** یه چارچوب سبک، خودکار و ماژولار برای روترهای **OpenWrt** هست که کارهایی مثل مدیریت شبکه، ترکیب چند کانکشن اینترنتی (**Multi-WAN**) و پخش بار بین مسیرهای مختلف (**Load Balancing**) رو ساده می‌کنه .

اگه مخزن‌های OpenWrt در دسترس نباشن یا دریافت بسته‌ها با مشکل روبه‌رو بشه، DayPass می‌تونه بسته‌های موردنیاز رو به‌صورت روزانه از **SourceForge** دریافت کنه و برای دانلود آماده کنه .

این ابزار با هر دو پکیج‌منیجر ورژن‌های جدید OpenWrt سازگاره :

- **OpenWrt 24.x** با `opkg`
- **OpenWrt 25.x** با `apk`

به این ترتیب، بسته‌های موردنیاز بر اساس ورژن OpenWrt و معماری سخت‌افزار روتر دریافت و نصب می‌شن .
