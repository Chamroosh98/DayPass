...
با پشتیبانی از **Passwall**، **Multi-WAN**، **Guest Network**، **DNS Manager** و **Clean IP**، کمک می‌کنه حتی تو شرایط فیلترینگ شدید، دسترسی پایدارتر و هوشمندتری به اینترنت داشته باشی. از انتخاب نود و شانت کردن ترافیک گرفته تا جایگزینی IPهای کلودفلر و پروفایل‌های آماده مسیریابی، همه چیز رو پوشش می‌ده.
...
- 🧭 **مدیریت DNS**  
  انتخاب مسیر DNS روتر بین حالت پیش‌فرض سیستم، DNS امن (DoT/DoH)، عبور DNS از تونل Passwall، یا حالت ترکیبی (Hybrid) .
...
چون DayPass به شکل پیوسته در حال به روزرسانیه، پس دو ورژن پایدار و آزمایشی داریم! ورژن پایدار امکانات کمتری داره ولی کاملاً پایداره. ورژن آزمایشی تقریباً هر روز آپدیت می‌شه و ویژگی های بیشتری داره، ولی ممکنه باگ داشته باشه!
...
اگه دستور `curl` از پیش روی روتر نصب کردی دستور زیر رو بزن :
...
با DayPass می‌تونی چندتا مسیر اینترنتی رو هم‌زمان روی روتر راه‌اندازی و مدیریت کنی.
...
> - **`RNDIS`:**
>
> استاندارد مایکروسافت برای شبیه‌سازی کارت شبکه روی USB ؛ بیشتر در گوشی‌های اندرویدی قدیمی و یسری مودم‌های دانگل بهره وری می‌شه.
>
> - **`CDC-Ethernet`:**
>
...
- **📡 اینترنت وای‌فای (`WWAN`)**  
  گرفتن اینترنت از یه روتر، مودم یا هات‌اسپات دیگه و رسوندنش به شبکه داخلی روتر.

این کانکشن‌ها به شکل خودکار در `mwan3` ثبت می‌شن و دو ویژگی اصلی رو در اختیارت می‌ذارن :

- **Failover :**

  اگر یکی از کانکشن‌ها قطع بشه، ترافیک به شکل خودکار از مسیر دیگه عبور می‌کنه.

- **Load Balancing :**

  ترافیک بین چند کانکشن پخش می‌شه تا از چند مسیر اینترنتی به شکل هم‌زمان بهره برداری بشه.
...
این بخش تو **ستینگ شبکه** قرار داره و مشخص می‌کنه روتر برای شبکه داخلی از چه مسیری DNS رو حل کنه.

❓ چکار میکنه؟

- **حالت سیستم (System Default) :** از DNS مودم / ISP استفاده می‌کنه؛ همون مسیر معمولی OpenWrt .
- **DNS امن (Secure) :** تا جایی که ممکنه از Resolverهای رمزنگاری‌شده (DoT/DoH) بهره می‌بره .
- **DNS از تونل :** کوئری‌های DNS رو از مسیر Passwall رد می‌کنه تا همراه ترافیک پروکسی برن (Passwall باید نصب باشه) .
- **هیبرید (پیشنهادی) :** اول مسیر تونل / DNS امن رو امتحان می‌کنه، اگه نشد سراغ Resolverهای عمومی مطمئن می‌ره .
